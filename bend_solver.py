...
def eq_bend_data(cloth):
    ...
    # remove based on zero weight vertex group
    
    '''
    if we're using it in modeling cloth we want
    a vertex group to define bend stiffness.
    This means everything will be in the group
    when it's turned to a cloth object
    It's default weight will be zero
    We have to check for a weight of more than
    zero to know if we should include it
    in the edges.
...
def abstract_bend(cloth):
    ...
    stretch = cloth.ob.MC_props.bend * .3
...
def get_springs_2(cloth):
    ...
    cloth.basic_v_fancy = cloth.basic_set[:,0]
    print(cloth.basic_set)
    print(cloth.basic_v_fancy)
...
def create_instance(ob=None):
    ...
    if 'MC_bend_stiff' not in ob.vertex_groups:
        ob.vertex_groups.new(name='MC_bend_stiff')

    # stuff ---------------
    cloth.obm = get_bmesh(ob)
    cloth.pin = get_weights(ob, 'MC_pin', obm=cloth.obm)[:, nax]
    cloth.bend_group = get_weights(ob, 'MC_bend_stiff', obm=cloth.obm)[:, nax]
    ...
    # abstract bend setup:
    if bpy.context.scene.MC_seam_wrangler:
...
def update_groups(cloth, obm=None, geometry=False):
    ...
    cloth.bend_group = get_weights(ob, 'MC_bend_stiff', obm=obm)[:, nax]
