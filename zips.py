def connect_panels(self, s_norm_val=1.0, offset_steps=0, correct_rotation=True, reverse=False):
    ...
    left_zipper_vert_ptrs = left_path#[::-1]
    right_zipper_vert_ptrs = right_path[::-1]
    ...
    co = np.array([ob.data.vertices[v].co for v in left_zipper_vert_ptrs])
    vecs = co[1:] - co[:-1]
...
def get_co_with_modifiers(ob, types=[], names=[], include_mesh=False, dg=None, att='co', proxy_only=False):
    ...
    if debug:
        ...
        if not (type_check & name_check):
            logging.info("!!! Warning. Mods not listed correctly !!!")
            logging.info("!!! Warning. Mods not listed correctly !!!")
            return
...
def auto_flip(co):
    ...
    assuming -z is down and reverses curve."""
    if co[-1][2] < co[0][2]:
        logging.info("Flipped the zipper curve direction.")
        return True
            ...
            if zob.parent is not None:
                if zob.parent.type == 'ARMATURE':
                    ...
                    try:
                        ...
                        logging.info('tried to remove constraint. Already removed.')
                if zob.parent.type != 'ARMATURE':
                    zob.parent = None
        return None, None
...
def cb_zipper_pull(self, context):
#def cb_zipper_pull(self, context, ob, s_normal=1):
    """Set this object as the zipper pull"""
    #zob = self.zipper_pull
    zob = ob
    order = ['middle_path', 'right_path', 'left_path']
    ...
    if curve is not None:
        ...
        length = total_curve_length(curve)
        #offset = (self.zipper_pull_offset * length) - x_dif
        offset = (s_normal * length) - x_dif
        displace.direction = 'X'
        displace.strength = offset


#def _cb_zipper_pull(self, context):
def _cb_zipper_pull(self, context, ob, s_normal=1):
    """Set this object as the zipper pull"""
    #zob = self.zipper_pull
    zob = ob
    order = ['middle_path', 'right_path', 'left_path']
...
def register():
    ...
    for cls in classes:
        try:
            ...
            logging.info('zipper_utils Classes already registered')
...
def place_zipper_on_garment(garment, left_path=None, right_path=None, zipper_pull_normal=1):
    ...
    if 'zipper_slider_body' in obs:
        ob = obs['zipper_slider_body']
        #self.zipper_pull = ob
        _cb_zipper_pull(self, bpy.context, ob, s_normal=1)
        ob.name = ob.name + '000'
